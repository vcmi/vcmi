			...
			for(AIPath & path : pathCache)
			{
				...
				if(NK2AI_GRAPH_TRACE_LEVEL >= 2 && updated)
				{
					logAi->trace(
						...
						fromObj ? fromObj->getObjectName().toString(&aiNk->translator()) : "J", from.toString(),
						"Boat", pos.toString(),
						pos.toString(),
						path.movementCost());
				}
	...
	for(AIPath & path1 : pathCache)
	{
		for(AIPath & path2 : pathCache)
		{
			...
			if(NK2AI_GRAPH_TRACE_LEVEL >= 2 && updated)
			{
				logAi->trace(
					"Connected %s[%s] -> %s[%s] through [%s], cost %2f",
					obj1 ? obj1->getObjectName().toString(&aiNk->translator()) : "J", pos1.toString(),
					obj2 ? obj2->getObjectName().toString(&aiNk->translator()) : "J", pos2.toString(),
					pos.toString(),
					path1.movementCost() + path2.movementCost());
			}
		}
