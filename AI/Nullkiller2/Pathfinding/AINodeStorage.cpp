	...
	TVector & output)
{
	...
	for (const auto & spell : LIBRARY->spellh->objects)
	{
		...
		for(const CGTownInstance * targetTown : towns)
		{
			...
			{
#if NK2AI_PATHFINDER_TRACE_LEVEL >= 1
				logAi->trace("Adding town portal node at %s", targetTown->getNameTextID());
#endif
