...
{
	LOG_TRACE_PARAMS(logAi, "Hero %s and object %s at %s", heroPtr->getNameTextID() % obj->getObjectName().toString(&nullkiller->translator()) % obj->anchorPos().toString());
	switch(obj->ID)
	{
	case Obj::TOWN:
...
void AIGateway::battleStart(const BattleID & battleID, const CCreatureSet * army1, const CCreatureSet * army2, int3 tile, const CGHeroInstance * hero1, const CGHeroInstance * hero2, BattleSide side, bool replayAllowed)
{
	...
	battlename = boost::str(boost::format("Starting battle of %s attacking %s at %s") % (hero1 ? hero1->getNameTextID() : "a army") % (presumedEnemy ? presumedEnemy->getObjectName().toString(&nullkiller->translator()) : "unknown enemy") % tile.toString());
