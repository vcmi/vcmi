...
void logHitmap(PlayerColor playerID, DangerHitMapAnalyzer & data)
{
	...
	logVisual->updateWithLock(std::to_string(playerID) + ".danger.max", [&data](IVisualLogBuilder & b)
		{
	...
	logVisual->updateWithLock(std::to_string(playerID) + ".danger.fast", [&data](IVisualLogBuilder & b)
		{
			foreach_tile_pos([&b, &data](const int3 & pos)
				{
