...
ChainActor::ChainActor(const CGHeroInstance * hero, HeroRole heroRole, uint64_t chainMask)
	...
	initialMovement(hero->movement), initialTurn(0), 
	armyValue(hero->getArmyStrength()), 
	heroFightingStrength(hero->getFightingStrength()), actorExchangeCount(1),
	armyCost(), actorAction()
{
...
ChainActor::ChainActor(const ChainActor * carrier, const ChainActor * other, const CCreatureSet * heroArmy)
	...
	baseActor(this), carrierParent(carrier), otherParent(other), armyValue(heroArmy->getArmyStrength()), heroFightingStrength(carrier->heroFightingStrength),
...
ChainActor::ChainActor(const CGObjectInstance * obj, const CCreatureSet * creatureSet, uint64_t chainMask, int initialTurn)
	...
	armyValue(creatureSet->getArmyStrength()), heroFightingStrength(0), actorExchangeCount(1), armyCost(), actorAction()
{
...
{
	exchangeMap = std::make_unique<HeroExchangeMap>(this, ai);
	setupSpecialActors();
}
...
ExchangeResult HeroExchangeMap::tryExchangeNoLock(const ChainActor * other)
{
	...
	HeroExchangeArmy * newArmy = nullptr;
