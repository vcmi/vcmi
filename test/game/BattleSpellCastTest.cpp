...
// ===========================================================================
// Spell-magnitude specialties (SPECIAL_SPELL_LEV, SPECIFIC_SPELL_DAMAGE)
//
...
//
// SPECIFIC_SPELL_DAMAGE is a flat base% regardless of level. For SPECIAL_SPELL_LEV
// the H3-correct bonus is  base% * floor(heroLevel / targetLevel)  ("base% for
// every <targetLevel> hero levels"). The engine instead computes
// floor(base% * heroLevel / targetLevel) (val is multiplied by hero level first,
// then divided by creature level in getSpellBonus), so cases where targetLevel
// does not divide heroLevel expose that rounding gap.
// ===========================================================================
...
TEST_P(SpellMagnitudeSpecialty, matchesBaseline)
{
	...
	const int percent = spec->type == BonusType::SPECIAL_SPELL_LEV
...
INSTANTIATE_TEST_SUITE_P(Heroes, SpellMagnitudeSpecialty, ::testing::Values(
	...
	// Higher-tier targets with heroLevel not divisible by target level: the correct
	// base%*floor(L/level) differs from the engine's floor(base%*L/level). These
...
// Both the engine paths under test (SPECIAL_SPELL_LEV for Astral's Hypnotize,
// TIMES_HERO_LEVEL_DIVIDE_STACK_LEVEL for Adela's Bless) instead compute
// floor(3 * heroLevel / targetLevel), so target levels that do not divide the
// hero level expose the rounding gap (see report).
// ===========================================================================
