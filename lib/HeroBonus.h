...
#define BONUS_TREE_DESERIALIZATION_FIX if(!h.saving && h.smartPointerSerialization) \
		deserializationFix();
...
#undef BONUS_NAME
	...
	enum BonusDuration //when bonus is automatically removed
	{
		...
		//at the end of day
		ONE_WEEK = 8,
		//at the end of week (bonus lasts till the end of week, thats NOT 7 days
		N_TURNS = 16,
		//used during battles, after battle bonus is always removed
		N_DAYS = 32,
		UNTIL_BEING_ATTACKED = 64,
		/*removed after attack and counterattacks are performed*/
		UNTIL_ATTACK = 128,
		/*removed after attack and counterattacks are performed*/
		STACK_GETS_TURN = 256,
		/*removed when stack gets its turn - used for defensive stance*/
...
#undef BONUS_SOURCE
	...
	enum LimitEffect
	{
		...
		ONLY_MELEE_FIGHT,
		//used to mark bonuses for attack/defense primary skills from spells like Precision (distance only)
		ONLY_ENEMY_ARMY
	};
