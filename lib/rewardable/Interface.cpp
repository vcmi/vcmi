...
void Rewardable::Interface::grantRewardWithMessage(IGameEventCallback & gameEvents, const CGHeroInstance * contextHero, int index, bool markAsVisit) const
{
	...
	logGlobal->debug("Granting reward %d. Message says: %s", index, vi.message.toString(LIBRARY->staticTexts()));
	// show message only if it is not empty or in infobox
	if (configuration.infoWindowType != EInfoWindowMode::MODAL || !vi.message.toString(LIBRARY->staticTexts()).empty())
	{
		InfoWindow iw;
		...
		if(!iw.components.empty() || !iw.text.toString(LIBRARY->staticTexts()).empty())
