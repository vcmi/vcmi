...
{
	std::set<SHexField> hostileCreaturePositions;
	std::set<SHexField> friendlyCreaturePositions; //for Dragon Breath
	template <typename Handler> void serialize(Handler &h, const int version)
	{
...
struct DLL_LINKAGE BattleInfo : public CBonusSystemNode
{
	...
	CStack * getStackT(SHexField tileID, bool onlyAlive = true);
	const CStack * getStackT(SHexField tileID, bool onlyAlive = true) const;
	void getAccessibilityMap(bool *accessibility, bool twoHex, bool attackerOwned, bool addOccupiable, std::set<SHexField> & occupyable, bool flying, const CStack* stackToOmmit = NULL) const; //send pointer to at least 187 allocated bytes
	static bool isAccessible(SHexField hex, bool * accessibility, bool twoHex, bool attackerOwned, bool flying, bool lastPos); //helper for makeBFS
	int getAvaliableHex(TCreature creID, bool attackerOwned, int initialPos = -1) const; //find place for summon / clone effects
	void makeBFS(SHexField start, bool*accessibility, SHexField *predecessor, int *dists, bool twoHex, bool attackerOwned, bool flying, bool fillPredecessors) const; //*accessibility must be prepared bool[187] array; last two pointers must point to the at least 187-elements int arrays - there is written result
	std::pair< std::vector<SHexField>, int > getPath(SHexField start, SHexField dest, bool*accessibility, bool flyingCreature, bool twoHex, bool attackerOwned); //returned value: pair<path, length>; length may be different than number of elements in path since flying vreatures jump between distant hexes
	std::vector<SHexField> getAccessibility(const CStack * stack, bool addOccupiable, std::vector<SHexField> * attackable = NULL) const; //returns vector of accessible tiles (taking into account the creature range)
	...
	std::set<CStack*> getAttackedCreatures(const CSpell * s, int skillLevel, ui8 attackerOwner, SHexField destinationTile); //calculates stack affected by given spell
	void getPotentiallyAttackableHexes(AttackableTiles &at, const CStack* attacker, SHexField destinationTile, SHexField attackerPos); //hexes around target that could be attacked in melee
	std::set<CStack*> getAttackedCreatures(const CStack* attacker, SHexField destinationTile, SHexField attackerPos = SHexField::INVALID); //calculates range of multi-hex attacks
	std::set<SHexField> getAttackedHexes(const CStack* attacker, SHexField destinationTile, SHexField attackerPos = SHexField::INVALID); //calculates range of multi-hex attacks
	std::set<CStack*> getAdjacentCreatures (const CStack * stack) const;
	static int calculateSpellDuration(const CSpell * spell, const CGHeroInstance * caster, int usedSpellPower);
	CStack * generateNewStack(const CStackInstance &base, int stackID, bool attackerOwned, int slot, SHexField position) const; //helper for CGameHandler::setupBattle and spells addign new stacks to the battlefield
	CStack * generateNewStack(const CStackBasicDescriptor &base, int stackID, bool attackerOwned, int slot, SHexField position) const; //helper for CGameHandler::setupBattle and spells addign new stacks to the battlefield
	ui32 getSpellCost(const CSpell * sp, const CGHeroInstance * caster) const; //returns cost of given spell
	int hexToWallPart(SHexField hex) const; //returns part of destructible wall / gate / keep under given hex or -1 if not found
	int lineToWallHex(int line) const; //returns hex with wall in given line
	std::pair<const CStack *, SHexField> getNearestStack(const CStack * closest, boost::logic::tribool attackerOwned) const; //if attackerOwned is indetermnate, returened stack is of any owner; hex is the number of hex we should be looking from; returns (nerarest creature, predecessorHex)
	...
	si8 hasDistancePenalty(const CStack * stackID, SHexField destHex) const; //determines if given stack has distance penalty shooting given pos
	si8 sameSideOfWall(int pos1, int pos2) const; //determines if given positions are on the same side of wall
	si8 hasWallPenalty(const CStack * stack, SHexField destHex) const; //determines if given stack has wall penalty shooting given pos
	si8 canTeleportTo(const CStack * stack, SHexField destHex, int telportLevel) const; //determines if given stack can teleport to given place
	bool battleCanShoot(const CStack * stack, SHexField dest) const; //determines if stack with given ID shoot at the selected destination
	...
	ESpellCastProblem::ESpellCastProblem battleCanCastThisSpellHere(int player, const CSpell * spell, ECastingMode::ECastingMode mode, SHexField dest) const; //checks if given player can cast given spell at given tile in given mode
	...
	const CStack * battleGetStack(SHexField pos, bool onlyAlive); //returns stack at given tile
	...
	bool isInTacticRange( SHexField dest ) const;
...
public:
	...
	SHexField position; //position on battlefield; -2 - keep, -3 - lower tower, -4 - upper tower
	...
	static bool isMeleeAttackPossible(const CStack * attacker, const CStack * defender, SHexField attackerPos = SHexField::INVALID, SHexField defenderPos = SHexField::INVALID);

	bool doubleWide() const;
	SHexField occupiedHex() const; //returns number of occupied hex (not the position) if stack is double wide; otherwise -1
	std::vector<SHexField> getHexes() const; //up to two occupied hexes, starting from front
	bool coversPos(SHexField position) const; //checks also if unit is double-wide
	std::vector<SHexField> getSurroundingHexes(SHexField attackerPos = SHexField::INVALID) const; // get six or 8 surrounding hexes depending on creature size
