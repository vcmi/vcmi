...
DLL_LINKAGE void NewObject::applyGs(CGameState *gs)
{
	...
	if(ID == Obj::BOAT && !gs->isInTheMap(pos)) //special handling for bug #3060 - pos outside map but visitablePos is not
	{
		...
		testObject.pos = pos;
		testObject.appearance = VLC->objtypeh->getHandlerFor(ID, subID)->getTemplates(CTerrainType("water")).front();
	...
	case Obj::BOAT:
		o = new CGBoat();
		terrainType = CTerrainType("water"); //TODO: either boat should only spawn on water, or all water objects should be handled this way
		break;
