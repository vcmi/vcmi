...
int CMapGenerator::deriveDeterministicSeed(int zoneId, std::string_view unitName, std::string_view streamTag) const
{
	...
	const uint32_t mixed = static_cast<uint32_t>(hash ^ (hash >> 32));
	const int deterministicSeed = static_cast<int>(mixed & 0x7fffffff);
...
void CMapGenerator::fillZones()
{
	...
	map->getMap(this).grailPos = *RandomGeneratorUtil::nextItem(grailZone->freePaths()->getTilesVector(), *rand);
