...
struct DLL_LINKAGE CObstacleInfo
{
	...
	std::vector<SHexField> getBlocked(SHexField hex) const; //returns vector of hexes blocked by obstacle when it's placed on hex 'hex'
	SHexField getMaxBlocked(SHexField hex) const; //returns maximal hex (max number) covered by this obstacle
	template <typename Handler> void serialize(Handler &h, const int version)
	{
