...
protected:
	bool check(const Mechanics * m, const battle::Unit * target) const override
	{
		...
		auto levelImmunities = target->getBonuses(Selector::type(Bonus::LEVEL_SPELL_IMMUNITY).And(Selector::info(1)), cachingStr.str());
...
protected:
	bool check(const Mechanics * m, const battle::Unit * target) const override
	{
		auto levelImmunities = target->getBonuses(Selector::type(Bonus::LEVEL_SPELL_IMMUNITY));
		return levelImmunities->size() == 0 ||
		levelImmunities->totalValue() < m->getSpellLevel() ||
		m->getSpellLevel() <= 0;
	}
