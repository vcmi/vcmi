...
GrowthInfo CGTownInstance::getGrowthInfo(int level) const
{
	...
	auto bonuses = getBonuses(Selector::type(Bonus::CREATURE_GROWTH).And(Selector::subtype(level)));
	for(const std::shared_ptr<Bonus> b : *bonuses)
		ret.entries.push_back(GrowthInfo::Entry(b->val, b->Description()));

	//statue-of-legion-like bonus: % to base+castle
	auto bonuses2 = getBonuses(Selector::type(Bonus::CREATURE_GROWTH_PERCENT));
