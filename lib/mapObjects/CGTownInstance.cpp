...
CCreGenLeveledInfo::CCreGenLeveledInfo()
	: CSpecObjInfo(), minLevel(0), maxLevel(7)
{
...
void CGDwelling::initRandomObjectInfo()
{
	...
	case Obj::RANDOM_DWELLING:
		info = new CCreGenLeveledCastleInfo();
		break;
	case Obj::RANDOM_DWELLING_LVL:
		info = new CCreGenAsCastleInfo();
		break;
	case Obj::RANDOM_DWELLING_FACTION:
		info = new CCreGenLeveledInfo();
		break;
	}
...
void CGDwelling::heroAcceptsCreatures(const CGHeroInstance * h) const
{
	...
	else
	{
		...
		ow.id1 = id.getNum();
		ow.id2 = h->id.getNum();
		ow.window = (ID == Obj::CREATURE_GENERATOR1 || ID == Obj::REFUGEE_CAMP) ? OpenWindow::RECRUITMENT_FIRST : OpenWindow::RECRUITMENT_ALL;
		cb->sendAndApply(&ow);
	}
}
