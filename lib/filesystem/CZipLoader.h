...
public:
	 ...
	 */
	CZipStream(const boost::filesystem::path & archive, unz_file_pos filepos);
	~CZipStream();
...
class DLL_LINKAGE CZipLoader : public ISimpleResourceLoader
{
	...
	std::unordered_map<ResourceID, unz_file_pos> listFiles(const std::string & mountPoint, const boost::filesystem::path &archive);
public:
