...
- MSVC v142 (VS 2019), specifically with compiler v19.29 - for Intel / AMD CPU
...
Alternatively, you may of course install the required toolset from the Visual Studio Installer. For example, for Intel / AMD CPU that would be *MSVC v142 - VS 2019 C++ x64/x86 build tools (v14.29-16.11)*.
...
1. Now you need to run a script* from the Conan directory that you passed to `conan install` (the one that you passed in `--output-folder` parameter). For example, if you passed `conan-generated`, then the script will be in `source\conan-generated`.
    - for CMD: `source\conan-generated\conanrun.bat`
    - for Powershell: `source\conan-generated\conanrun.ps1`. If it gives an error, also run `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy Unrestricted`
2. Create VS solution: `cmake -S source -B build --toolchain source\conan-generated\conan_toolchain.cmake`

\* This script sets up `PATH` required for Qt build-time tools (`moc`, `uic` etc.) that run during CMake configure and build steps. Those tools depend on `zlib.dll` that was built with Conan, therefore its directory must be in `PATH`. As an alternative to running `conanrun.bat` you may try [this workaround](https://github.com/kambala-decapitator/ExeWrapper).
...
5. VCMI will be built in `%VCMI_DIR%/build/bin/<config>` folder where `<config>` is e.g. `Debug`. To launch the built executables from a file manager, use respective `bat` files, e.g. `VCMI_launcher.bat`.
