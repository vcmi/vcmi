                   ...
                   const std::set<std::string> & knownTypes)
{
	...
	std::ofstream out((outDir / (typeName + ".md")).c_str());
	out << content;
}
...
{
	std::ofstream out((outDir / (group.name + ".md")).c_str());
...
{
	std::ofstream out((outDir / "API.md").c_str());
...
void exportLuaApiDocs(const boost::filesystem::path & outDir)
{
	...
	std::ofstream lua((outDir / "api.lua").c_str());
	lua << "---@meta\n"
