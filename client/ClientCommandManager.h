...
class ClientCommandManager //take mantis #2292 issue about account if thinking about handling cheats from command-line
{
	...
	void handleLoadCommand(std::istringstream & singleWordBuffer) const;
	...
	void handleAutoskipCommand() const;
	...
	void handleCrashCommand() const;

	// Prints in Chat the given message
	void printCommandMessage(const std::string &commandMessage, ELogLevel::ELogLevel messageType = ELogLevel::NOT_SET) const;
	void giveTurn(const PlayerColor &color) const;
