...
{
	if(bindings.size() != 1 || !ControllerPrompt::usesRuntimeFaceLabel(family)
		|| !ControllerPrompt::isFaceButtonBinding(bindings.front()))
		return "";
	...
	void showAll(Canvas & to) override
	{
		...
		if(!spriteLabel.empty())
		{
			to.drawText(pos.topLeft() + promptSpritePosition + Point(12, 12), EFonts::FONT_SMALL,
				ControllerPrompt::runtimeFaceLabelColor(state),
...
void CControllerActionButton::refreshControllerPrompt()
{
	...
	const auto currentBindings = ENGINE->shortcuts().getJoystickButtonBindings(assignedKey, currentFamily);
